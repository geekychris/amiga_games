...
/*
 * Rock Blaster - Drawing routines using Amiga graphics.library
 */
...
void draw_title(struct RastPort *rp)
{
    ...
    /* --- Title using system font for performance --- */
    SetBPen(rp, 0);
    SetAPen(rp, 10); /* yellow */
    Move(rp, cx - 48, 75);
    Text(rp, (STRPTR)"ROCK", 4);

    SetAPen(rp, 1); /* white */
    Move(rp, cx - 56, 100);
    Text(rp, (STRPTR)"BLASTER", 7);
...
void draw_gameover(struct RastPort *rp, LONG score)
{
    ...
    SetAPen(rp, 8); /* red */
    SetBPen(rp, 0);
    Move(rp, cx - 36, SCREEN_H / 2 - 30);
    Text(rp, (STRPTR)"GAME OVER", 9);
    (void)w;
