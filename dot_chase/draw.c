...
static void draw_muncher(struct RastPort *rp, GameState *gs)
{
    WORD px = MAZE_OX + gs->muncher.pixel_x;
    WORD py = MAZE_OY + gs->muncher.pixel_y;
    WORD cx = px + 3;
    WORD cy = py + 3;

    SetAPen(rp, COL_YELLOW);

    if (!gs->muncher.mouth_open) {
        safe_rect(rp, cx - 3, cy - 2, cx + 3, cy + 2);
        safe_rect(rp, cx - 2, cy - 3, cx + 2, cy + 3);
    } else {
        switch (gs->muncher.dir) {
        case DIR_RIGHT:
...
void draw_game(struct RastPort *rp, GameState *gs)
{
    ...
    if (maze_dirty > 0) {
        ...
        prev_pac_x = gs->muncher.pixel_x;
        prev_pac_y = gs->muncher.pixel_y;
...
void draw_title(struct RastPort *rp, GameState *gs)
{
    ...
    SetRast(rp, COL_BLACK);
    draw_text_centered(rp, 20, COL_YELLOW, "DOT_CHASE", 3);
